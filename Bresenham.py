...
def Bresenham(initial_point, final_point):
    ...
    points = []
    if x2 == x1:
        points = (YLine(x1, [y1,y2]))
        return points
    else: 
        m = (y2 - y1) / (x2 - x1)
        swipe_coordinates, swipe_x, swipe_y,new_x1,new_x2,new_y1,new_y2 = Reflection( x1,x2,y1,y2,m)
        ...
        while x1 < x2:
            ...
            e += m
            points.append([x1,y1])
        ReflectionInverse( swipe_coordinates, swipe_x, swipe_y, points)
    return points
...
def Draw(points, poli = False, color = '#f00'):
        if poli:
            for i in points:
                for j in i:
                    PaintPixel(j[0], j[1], color)
        else: 
            for point in points:
                PaintPixel(point[0], point[1], color)
...
def Poliline(*args):
    args = args[0]
    group = [Bresenham(args[i], args[i+1]) for i in range(len(args)-1)]
    group.append(Bresenham(args[len(args)-1],args[0]))
    return group
